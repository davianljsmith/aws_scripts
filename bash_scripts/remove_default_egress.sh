...
#Get Security Rule ID
    c=`aws ec2 describe-security-group-rules --filter Name="group-id",Values=${b} --region ${i} | grep SECURITYGROUPRULES | grep True | awk {'print $8'}`;

#Delete rule
    aws ec2 revoke-security-group-egress --group-id ${b} --security-group-rule-ids ${c} --region ${i};
